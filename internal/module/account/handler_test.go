...
func TestAccountHandler_ListAccounts(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "successful list with all parameters",
			queryParams: "?page=1&limit=5&email=test@example.com&username=testuser&is_active=true",
			setupMock: func(mockService *MockAccountService) {
				responses := []AccountResponse{
					*CreateTestAccountResponse(),
					*CreateTestAccountResponse(func(ar *AccountResponse) {
						ar.Email = "test2@example.com"
						ar.Username = "testuser2"
					}),
				}

				result := &mongo.PaginatedResult[AccountResponse]{
					Data:       responses,
					Total:      2,
					Page:       1,
					Limit:      5,
				...
				mockService.On("ListAccounts", mock.Anything, mock.AnythingOfType("*account.ListAccountsRequest")).Return(result, nil)
			},
			expectedStatus: fiber.StatusOK,
			expectError:    false,
		},
		{
			name:        "successful list with minimal parameters",
		...
		},
		{
			...
			setupMock: func(mockService *MockAccountService) {
				mockService.On("ListAccounts", mock.Anything, mock.AnythingOfType("*account.ListAccountsRequest")).Return((*mongo.PaginatedResult[AccountResponse])(nil), errors.New("validation error"))
			},
			expectedStatus: fiber.StatusInternalServerError,
			expectError:    true,
		...
		},
		{
			...
			queryParams: "?page=1&limit=10",
			setupMock: func(mockService *MockAccountService) {
				mockService.On("ListAccounts", mock.Anything, mock.AnythingOfType("*account.ListAccountsRequest")).Return((*mongo.PaginatedResult[AccountResponse])(nil), errors.New("database error"))
			},
			expectedStatus: fiber.StatusInternalServerError,
			expectError:    true,
			errorContains:  "Failed to list accounts",
		...
		{
			name:        "default pagination when not provided",
			queryParams: "",
			setupMock: func(mockService *MockAccountService) {
				result := &mongo.PaginatedResult[AccountResponse]{
					Data:       []AccountResponse{},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			handler := NewAccountHandler(mockService)
			app := setupTestApp()
			app.Get("/accounts", handler.ListAccounts)
